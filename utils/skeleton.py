...
class SkeletonProcessor:
    ...
    def _normalize(self, sequence: np.ndarray) -> np.ndarray:
        ...
        # 缩放：优先肩宽，肩宽过小时自动回退到躯干长度/髋宽
        left_shoulder = normalized[:, self.JOINT_IDX["left_shoulder"], :2]
        right_shoulder = normalized[:, self.JOINT_IDX["right_shoulder"], :2]
        left_hip_n = normalized[:, self.JOINT_IDX["left_hip"], :2]
        right_hip_n = normalized[:, self.JOINT_IDX["right_hip"], :2]

        shoulder_width = np.linalg.norm(left_shoulder - right_shoulder, axis=1)
        hip_width = np.linalg.norm(left_hip_n - right_hip_n, axis=1)
        shoulder_center = (left_shoulder + right_shoulder) / 2
        hip_center_n = (left_hip_n + right_hip_n) / 2
        torso_len = np.linalg.norm(shoulder_center - hip_center_n, axis=1)

        base_scale = shoulder_width.copy()
        fallback_scale = np.maximum(torso_len, hip_width)
        weak_shoulder_mask = shoulder_width < 8.0
        base_scale[weak_shoulder_mask] = fallback_scale[weak_shoulder_mask]

        valid_scale = base_scale[base_scale > 1e-6]
        if len(valid_scale) > 0:
            stable_floor = np.percentile(valid_scale, 20) * 0.5
            stable_floor = max(stable_floor, 1e-3)
        else:
            stable_floor = 1e-3

        scale = np.maximum(base_scale, stable_floor)
        ...
        for t in range(T):
            normalized[t, :, :2] = normalized[t, :, :2] @ rot_matrix.T
    ...
    def _analyze_squat(self, features: np.ndarray) -> Dict:
        ...
        metrics["knee_collapse"] = float(
            np.max([knee_collapse_left.max(), knee_collapse_right.max()])
        )
        metrics["knee_toe_alignment"] = metrics["knee_collapse"]
        ...
        metrics["back_angle"] = metrics["torso_angle"]

        descend_frames = max(bottom_idx, 1)
        ascend_frames = max(len(features) - bottom_idx, 1)
        metrics["speed_symmetry"] = float(descend_frames / ascend_frames)
    ...
    def _analyze_situp(self, features: np.ndarray) -> Dict:
        ...
        metrics["touch_position"] = metrics["touch_distance"]

        # 臀部稳定性
        hip_variance = np.var(hip_y)
        metrics["hip_stability"] = float(hip_variance)
        metrics["down_control"] = float((len(features) - max_idx) / 30.0)
    ...
    def _analyze_jump_rope(self, features: np.ndarray) -> Dict:
        ...
        if len(peaks) > 1:
            ...
            metrics["rhythm_regularity"] = float(
    ...
    def _analyze_long_jump(self, features: np.ndarray) -> Dict:
        ...
        if takeoff_idx > 0 and takeoff_idx < len(features) - 1:
            ...
            hip_velocity_x = (
                features[takeoff_idx, self.JOINT_IDX["left_hip"], 0]
                - features[takeoff_idx - 1, self.JOINT_IDX["left_hip"], 0]
            )
            takeoff_angle = np.degrees(
                np.arctan2(hip_velocity_y, abs(hip_velocity_x) + 1e-6)
            )
        ...
        metrics["landing_balance"] = metrics["landing_stability"]

        flight_end = max(takeoff_idx + 1, 3 * len(features) // 4)
        flight_slice = features[takeoff_idx:flight_end]
        if len(flight_slice) > 0:
            metrics["flight_posture"] = float(np.mean(flight_slice[:, 11, 5]))
        else:
            metrics["flight_posture"] = 160.0
    ...
    def _analyze_pullup(self, features: np.ndarray) -> Dict:
        ...
        left_elbow = features[:, 7, 3]
        right_elbow = features[:, 8, 3]
        metrics["min_elbow_angle"] = float(
            np.min([left_elbow.min(), right_elbow.min()])
        )
        metrics["max_elbow_angle"] = float(
            np.max([left_elbow.max(), right_elbow.max()])
        )
        metrics["arm_extension"] = metrics["max_elbow_angle"]
        ...
        metrics["pull_speed"] = metrics["pull_time"]
    ...
    def compute_score(metrics: Dict, action_config: Dict) -> Tuple[float, Dict]:
        ...
        for param_name, standard in standards.items():
            ...
            if min_val <= value <= max_val:
                ...
                # 超出范围，给低分
                score = max(
                    0, 50 - abs(value - ideal) / (max_val - min_val + 1e-6) * 50
                )
    ...
    for i, ax in enumerate(axes):
        ...
        for pair in processor.SKELETON_PAIRS:
            if pair[0] < N and pair[1] < N:
                ax.plot(
                    [coords[pair[0], 0], coords[pair[1], 0]],
                    [coords[pair[0], 1], coords[pair[1], 1]],
                    "g-",
                    linewidth=2,
                )
