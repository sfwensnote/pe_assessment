    ...
    def __init__(
        ...
        if edge_importance_weighting:
            self.edge_importance = nn.ParameterList(
                [nn.Parameter(torch.ones(self.A.size())) for _ in self.st_gcn_layers]
            )
        else:
...
class STGCNBlock(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_conv = nn.Conv2d(
            out_ch, out_ch, (kernel_size, 1), stride=(stride, 1), padding=(padding, 0)
        )
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            A: [N, N] 邻接矩阵
        """
        # 图卷积操作: 在每个时间步上按关节维做 A @ X
        # [B, C, T, N] x [N, N] -> [B, C, T, N]
        x_graph = torch.einsum("bctn,nm->bctm", x, A)
...
class TemporalUNet(nn.Module):
    ...
    def __init__(
        ...
        if len(hidden_dims) < 2:
            raise ValueError("hidden_dims must contain at least 2 levels")
        ...
        for i in range(len(hidden_dims) - 1):
            self.encoders.append(self._conv_block(hidden_dims[i], hidden_dims[i + 1]))
        ...
        decoder_skip_channels = list(reversed(hidden_dims[1:]))
        decoder_in_channels = hidden_dims[-1] * 2

        for skip_channels in decoder_skip_channels:
            self.upconvs.append(
                nn.ConvTranspose1d(
                    decoder_in_channels,
                    skip_channels,
            ...
            self.decoders.append(self._conv_block(skip_channels * 2, skip_channels))
            decoder_in_channels = skip_channels

        # 输出层
        self.out = nn.Conv1d(decoder_in_channels, num_phases, 1)

        # 时序注意力
        self.temporal_attention = TemporalAttention(decoder_in_channels)
...
class QualityNet(nn.Module):
    ...
    def __init__(
        ...
        self.feature_fusion = nn.Sequential(
            nn.Linear(256, feature_dim), nn.ReLU(inplace=True), nn.Dropout(0.3)
        )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # 多任务预测（分数采用 0-1 标准化输出）
        overall_score = self.score_head(features)
...
class LiteQualityNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {
            "overall": self.score_head(features),
...
def export_to_onnx(
    model: nn.Module, save_path: str, input_size: tuple = (1, 60, 17, 9)
):
    """导出模型为ONNX格式"""
    model.eval()
