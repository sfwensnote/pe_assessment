...
class SkeletonAugmentor:
    ...
    def random_translation(self, features: np.ndarray) -> np.ndarray:
        ...
        translation = np.random.uniform(
            -self.translation_range, self.translation_range, size=2
        )
    ...
    def time_warp(
        ...
        if labels is not None:
            ...
            label_indices = np.rint(new_indices).astype(int)
            label_indices = np.clip(label_indices, 0, len(labels) - 1)
            warped_labels = labels[label_indices]

        # 截断或填充回原始长度
        if new_T >= T:
            features = new_features[:T]
            if warped_labels is not None:
                labels = warped_labels[:T]
        else:
            pad = T - new_T
            features = np.concatenate(
                [new_features, np.tile(new_features[-1:], (pad, 1, 1))]
            )
            if warped_labels is not None:
                labels = np.concatenate(
                    [warped_labels, np.tile(warped_labels[-1:], pad)]
                )
    ...
    def joint_masking(
        self, features: np.ndarray, num_joints_to_mask: int = 1
    ) -> np.ndarray:
        """
        随机遮罩关节点（模拟遮挡）
        """
...
class CutMixAugmentation:
    ...
    def __call__(
        ...
        mixed_features[:, start_idx : start_idx + cut_len] = features2[
            :, start_idx : start_idx + cut_len
        ]
...
def apply_augmentation_pipeline(
    features: np.ndarray, augmentation_config: dict
) -> np.ndarray:
    """
    根据配置应用增强管道
