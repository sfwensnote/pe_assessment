...
class AutoAnnotator:
    ...
    def annotate_pushup(self, features: np.ndarray) -> tuple:
        ...
        phases[(elbow_angles > down_threshold) & (elbow_angles <= ready_threshold)] = 1  # down
        phases[elbow_angles <= down_threshold] = 2  # bottom
        phases[(elbow_angles > down_threshold) & (np.arange(T) > bottom_idx)] = 3  # up
        phases[(elbow_angles > ready_threshold) & (np.arange(T) > bottom_idx)] = 4  # finish
        ...
        if hip_range > 0.15:
            quality['errors'].append('塌腰' if hip_y[bottom_idx] > np.mean(hip_y) else '撅臀')
    ...
    def annotate_situp(self, features: np.ndarray) -> tuple:
        ...
        quality = {
            'up_angle': float(np.degrees(np.arctan2(hip_y[0] - shoulder_y[top_idx], 0.5))),
    ...
    def process_action(self, action_type: str, skeleton_dir: Path, anno_dir: Path):
        """处理单个动作的标注"""
...
def main():
    ...
    parser.add_argument('--action', type=str, default=None,
                       help='指定标注的动作类型')
    parser.add_argument('--skeleton_dir', type=str, default=CONFIG['paths']['skeletons'],
                       help='骨骼数据目录')
    parser.add_argument('--anno_dir', type=str, default=CONFIG['paths']['annotations'],
                       help='标注输出目录')
    ...
    # 处理动作
    if args.action:
        annotator.process_action(args.action, skeleton_dir, anno_dir)
    else:
        total = 0
        for action_type in CONFIG['actions'].keys():
            count = annotator.process_action(action_type, skeleton_dir, anno_dir)
            total += count
