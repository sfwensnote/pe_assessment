...
def export_action_model(format: str, output_dir: Path, checkpoint_dir: Path):
    ...
    # 创建示例输入
    dummy_input = torch.randn(1, CONFIG['skeleton']['target_frames'], 
                             CONFIG['skeleton']['num_joints'], 9)
    ...
    model_info = {
        ...
        'input_shape': [1, CONFIG['skeleton']['target_frames'], 
                       CONFIG['skeleton']['num_joints'], 9],
...
def create_deploy_package(output_dir: Path, format: str):
    """创建部署包"""
    print("\n创建部署包...")
...
class OnnxAssessor:
    def __init__(self, model_dir='models'):
        ...
        # 加载配置
        with open(self.model_dir / '..' / 'config.yaml') as f:
...
def main():
    ...
    parser.add_argument('--checkpoint_dir', type=str,
                       default=CONFIG['paths']['checkpoints'],
                       help='检查点目录')
    ...
    # 创建部署包
    create_deploy_package(output_dir, args.format)
